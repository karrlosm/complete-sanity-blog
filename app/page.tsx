...
async function getRecentsPosts() {
  const query = `
...
export default async function Home() {
  ...
  const mostViewed: SimpleBlogCardProps[] = await getRecentsPosts()
  const categories: CategoryProps[] = await getAllCategories()

  return (
    <div className="relative w-full h-full pt-10 pb-5">
      <div className="mx-auto px-4 max-w-7xl">
        <div className="grid w-full grid-cols-[2fr_1fr] max-lg:grid-cols-1 gap-3">
      ...
      <div className="mb-5 mt-10 mx-auto px-4 max-w-7xl">
        ...
        <div className="flex justify-center items-center py-5">
          <Link href={'/blog'} className="cursor-pointer relative rounded-full group overflow-hidden px-4 py-2 border-2 border-primary-custom text-primary-custom font-semibold
            ...
            <span className="relative z-20 text-lg transition-colors duration-300 group-hover:text-white">
              Todas as notícias
            </span>
          </Link>
        </div>
      ...
      <div className="mx-auto pt-5 px-4 max-w-7xl">
        <div className="items-start my-5 flex flex-col">
